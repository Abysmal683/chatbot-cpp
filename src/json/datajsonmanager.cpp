...
QStringList DataJsonManager::loadGenres() {
    QJsonArray arr = loadArray(":/database/genres.json", "genres");
    QStringList result;
...
QStringList DataJsonManager::loadPlatforms() {
    QJsonArray arr = loadArray(":/database/platforms.json", "platforms");
    QStringList result;
    for (const auto &v : arr) result << v.toString();
    return result;
}
...
QStringList DataJsonManager::loadTags() {
    QJsonArray arr = loadArray(":/database/tags.json", "tags");
    QStringList result;
    for (const auto &v : arr) result << v.toString();
    return result;
}
